...
def test_get_pyramid_latex_representation():
    ...
    assert expected_out == get_pyramid_latex_representation(*input_values)
...
def test_convert_latex_to_image():
    ...
    image_bytes = convert_latex_to_image(latex_expression)
    produced_image = Image.open(image_bytes)
    original_image = Image.open("test/data/test-pyramid.png")

    assert imagehash.average_hash(produced_image) == imagehash.average_hash(
        original_image
    )
